...
public class EventServiceImp implements EventService {
    ...
    private Event updateEventFields(Event target, UpdateEventUserRequest source, Category category, Location location) {
        ...
        if (source.getStateAction() != null) {
            if (source.getStateAction().equals(StateAction.CANCEL_REVIEW)) target.setEventState(EventState.CANCELED);
            if (source.getStateAction().equals(StateAction.SEND_TO_REVIEW)) target.setEventState(EventState.PENDING);
        }
        return target;
    }
